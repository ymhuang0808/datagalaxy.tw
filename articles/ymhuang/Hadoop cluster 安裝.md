...
* **ApplicationManager** : 一個 application 送到 ResourceManager 後，ResourceManager 會啟動一個 ApplicationMaster 負責執行、監控和 ResourceManager 協調執行 application 所需的資源
...
`slaves` 設定檔是為了讓 master node 知道 Hadoop cluster 的 slave nodes，所以需要在 `slaves` 設定檔中加入 slave nodes 的主機名稱或 IP 位址：
...
因為 hadoop01 是 master 也是 slave，所以需要被加入到設定檔中。
...
把 Hadoop 相關的環境變數加到 hadoop 使用者中，在使用指令時，可以不用輸入完整路徑的指令，要將環境變數要**加入到所有的 nodes 中**。
...
#### 建立 NameNode 及 DataNode 路徑

NameNode 是儲存 DataNode 中 block 的 metadata，在上面第 2 個設定步驟中，在 `hdfs-site.xml` 中的 `dfs.datanode.data.dir` 及 `dfs.namenode.name.dir` 的設定值分別是 DataNode 與 NameNode 資料儲存的路徑，所以分別要在 DataNode 和 NameNode 建立儲存路徑。

這次實作是將 3 個 nodes 皆設為 DataNodes，所以要在 3 個 nodes 建立 DataNode 的資料儲存路徑： 

```
[hadoop@hadoop01 ~]$ /home/hadoop/datanode
[hadoop@hadoop01 ~]$ chmod 755 /home/hadoop/datanode
```

除了 DataNode 外，hadoop01 主機為 master node，也是 NameNode，所以須建立儲存 NameNode 資料路徑：

```
[hadoop@hadoop01 ~]$ /home/hadoop/namenode
[hadoop@hadoop01 ~]$ chmod 755 /home/hadoop/namenode
```
